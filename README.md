...
在修正分词后，BM25 可以完成词面召回，但在当前语料条件下仍会引入噪声：语料规模较小、来源领域分散、问题通常能由单个主题文档回答，向量检索已经足够稳定。目前混合检索评测结果如下：

| 指标 | 向量基线 | 混合检索（向量权重/BM25权重=0.7/0.3） |
| --- | ---: | ---: |
| Faithfulness | 0.8172 | 0.8375 |
| Answer Relevancy | 0.7979 | 0.7374 |
| Context Precision | 0.8444 | 0.8640 |
| Context Recall | 0.9500 | 0.9233 |
